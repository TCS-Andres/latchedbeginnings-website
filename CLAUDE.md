...
- **Routes:** `src/app/api/chat/route.ts` (streaming, Vercel AI SDK v6 + direct `@ai-sdk/anthropic`, model `claude-sonnet-4-6`, prompt-cached system message).
- **Widget:** `src/components/global/ChatAgent.tsx` (`useChat`, ~2.6s typing delay, typing dots, bubble animations, callback form). The chat callback and the appointment form (`src/components/forms/AppointmentForm.tsx`) both submit **client-side** to **Web3Forms** (`https://api.web3forms.com/submit`), which emails the lead to the inbox tied to the access key. Web3Forms blocks server-side calls on the free plan, so this must stay a browser fetch. Access key is embedded with a `NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY` override.
- **Env:** set `ANTHROPIC_API_KEY` in `.env.local` for the chat agent. Form delivery needs no server env (Web3Forms key is client-side); optionally set `NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY` to override. See `.env.example`.
